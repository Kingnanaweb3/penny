...
- Service methods keep accepting amounts in dollars, because callers send dollars.
- ledger.balance(account) keeps returning dollars.
- Internally, store and calculate every amount in whole cents (integers only).
- Add ledger.balanceCents(account) that returns the balance as an integer number of cents.
